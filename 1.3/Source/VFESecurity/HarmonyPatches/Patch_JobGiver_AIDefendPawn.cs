...
namespace VFESecurity
{
    ...
    public static class Patch_JobGiver_AIDefendPawn
    {

        [HarmonyPatch(typeof(JobGiver_AIDefendPawn), "TryFindShootingPosition")]
        public static class TryFindShootingPosition
        {
            ...
            {
                #if DEBUG
                    Log.Message("Transpiler start: JobGiver_AIDefendPawn.TryFindShootingPosition (1 match)");
                #endif
                ...
                for (int i = 0; i < instructionList.Count; i++)
                {
                    ...
                    {
                        #if DEBUG
                            Log.Message("JobGiver_AIDefendPawn.TryFindShootingPosition match 1 of 1");
                        #endif

                        yield return instruction; // verb.verbProps.range
                        yield return new CodeInstruction(OpCodes.Ldloc_0); // verb
